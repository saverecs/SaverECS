...
showOnly = (chart, b) ->
  ...
  chart.chartContainer.selectAll("rect").data(chart.chartData)
    ...
    .attr("width", (d) -> chart.xScale(d.domX[1]) \
    - chart.xScale(d.domX[0]) \
    - .5)
...
processJson = (json) ->
  ...
  traces = _.filter(json.traces, \
  (item) -> _.some(groups, (g) -> (+item[0].group) == g))
...
class Chart
  constructor: (data) ->
    ...
    #   'basis' smooths it the most, however, when working with
    #   a lot of data, this will slow it down
    # */
    key0 = data.data.key0
    key1 = data.data.key1
