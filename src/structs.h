...
struct transitionSpaceEx{
	...
	struct condition* guard[1024];
	struct condition* assignments[1024];
...
struct locationSpaceEx {
	...
	struct condition* invariants[100];
	struct condition* flows[100];
...
struct interval* extractJSONFeatureInterval(char *featureName, int l, int depth);
