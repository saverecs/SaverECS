...
{       
	...
	pressure_est = prev_pressure_est + ( Sampling_Time * c1 * (c23 * dmAf - mCf(prev_pressure_est) ) );
	integral_error = prev_integral_error + Sampling_Time * c14 * (c24 * airbyfuel_meas - c11) ;
	commanded_fuel_gps = ((1 + prev_integral_error + (c13 * ((c24 * airbyfuel_meas) - c11))) * mCf(prev_pressure_est)) / c11 ;	
